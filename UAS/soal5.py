...
if __name__ == "__main__":
    ...
    for x in dict_pembeli_produk:
        if(nama!=x):
            ...
            dict_nilai_kesamaan[x] = kemiripan

    list_pembeli_lain_terutut_kemiripan = sorted(dict_nilai_kesamaan,key=dict_nilai_kesamaan.__getitem__, reverse=True)

    print("Pembeli: " + nama)
    print("Nilai kemiripan dengan pembeli lain: ")
    for x in list_pembeli_lain_terutut_kemiripan:
        rekomen = rekomendasi_produk(dict_pembeli_produk[nama], dict_pembeli_produk[x])
        print(x,end=' = ')
        print(dict_nilai_kesamaan[x],end=' : ')
        print(rekomen)
